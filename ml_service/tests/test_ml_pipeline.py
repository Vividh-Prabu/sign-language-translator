...
# Suppress sklearn feature name validation warnings in test runner
...
class TestMLServicePipeline(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        """Runs once before all tests: ensure artifacts exist."""
        cls.model = load_model()
        cls.scaler = load_scaler()
        cls.expected_features = cls.scaler.n_features_in_
        cls.valid_sample = [0.12] * cls.expected_features

    def test_01_data_loading(self):
        """Test dataset loads correctly and contains no NaNs."""
        ...
        self.assertEqual(X.isnull().sum().sum(), 0, "Features contain NaN values.")

    def test_02_model_artifacts_loaded(self):
        """Test model and scaler are loaded properly."""
        self.assertIsNotNone(self.model, "SVM Model failed to load.")
        self.assertIsNotNone(self.scaler, "StandardScaler failed to load.")
        self.assertTrue(hasattr(self.model, "predict"), "Model object is missing predict method.")

    def test_03_prediction_valid_input_list(self):
        """Test prediction with standard Python list input."""
        pred = predict_gesture(self.valid_sample)
        self.assertIsInstance(pred, str, "Prediction output must be a string.")
        self.assertIn(pred, self.model.classes_, "Predicted label is not in trained classes.")
    ...
    def test_05_prediction_with_confidence(self):
        """Test confidence output ranges between 0.0 and 1.0."""
        pred, conf = predict_gesture_with_confidence(self.valid_sample)
        self.assertIsInstance(pred, str)
        self.assertIsInstance(conf, float)
        self.assertTrue(0.0 <= conf <= 1.0, f"Confidence {conf} out of bounds.")

    def test_06_wrong_feature_count_raises_error(self):
        """Test that passing wrong feature count raises ValueError."""
        invalid_sample = [0.1, 0.2]  # Only 2 features instead of 12
